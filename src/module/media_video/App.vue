<template>
  <div class="media_video">
      ...
      <div class="page-content-wrapper">
        <div class="page-content">
          <ul class="tab clear">
            ...
            <li class="list">
              <a href="candidate_list.html" class="active">/ &nbspVideo Management</a>
            </li>
          </ul>
          <div class="pic_logic">
    ...
    <div class="popup" v-show="edit_popup" style="height: 512px;margin-top: -256px">
      <span class="popup_top_tag">
        Edit
      ...
      <div class="edit_popup_con">
        ...
        <div class="form-group">
          ...
          <textarea name="" id="des" cols="30" rows="10" class="form-control" v-model="edit_des_val" style="padding: 6px 8px"></textarea>
        </div>
        <div class="form-group">
  ...
  export default{
    ...
    methods:{
      ...
      pageMaterial(page,pageSize,catesele){
        ...
        _this.$http.get(interfaceStr+'cc/material/page?access_token='+token+'&page='+page+'&pageSize='+pageSize+'&groupId='+catesele+'&materialType=1').then(function (res) {
      ...
      add_popup_confirm(){
        if(this.categoryName ==='' || this.retype === ''){
          ...
          /*this.categoryName ='';
           this.retype = '';*/
          var token = $.cookie('token');
          this.$http.post(interfaceStr+'cc/material/addgroup',{'access_token':token,'groupName':this.categoryName,'groupType':1},{emulateJSON:true}).then(function (res) {
            if(res.body.error_code===200) {
    ...
    mounted(){
      ...
      function reads(ran,file){
        ...
        formData.append('videoMediaFile',file);
        formData.append('groupId',_this.cateSele_val);
        formData.append('progressId',ran);
        formData.append('access_token',token);
        getProgess(ran);
        console.log(_this.progessNumbers);
        _this.$http.post(interfaceStr+'cc/blog/uploadVideo',formData,{headers: {'Content-Type': 'multipart/form-data'}}).then(function (res) {
          if(res.body.error_code===200){
      ...
      $('#inputFile').change(function () {
        ...
        for (var i = 0; i < fileList.length; i++) {
          if(!/\.(avi|dat|mpg|wmv|asf|rm|rmvb|mov|flv|mp4|3gp|dv|divx|qt|asx)$/i.test(fileList[i].name)){
      ...
      oDrag.addEventListener('drop', function (e) {
        ...
        var maxSize = 15000;
        if(file.length >= 6){
          alert('一次最多上传 5 个视频～ , 不好意思');
          return false
        }else {
          _this.rans=[];
          _this.progessNumbers = [];
          for (var i = 0; i < file.length; i++) {
            if(!/\.(avi|dat|mpg|wmv|asf|rm|rmvb|mov|flv|mp4|3gp|dv|divx|qt|asx)$/i.test(file[i].name)){
              alert('请上传avi、dat、mpg、wmv、asf、rm、rmvb、mov、flv、mp4、3gp、dv、divx、qt、asx格式的视频～')
              return false
      ...
      //获取所有分类
      this.$http.get(interfaceStr+'cc/material/grouplist?access_token='+token+'&type=1').then(function (res) {
        if(res.body.error_code===200){
      ...
      /*function pageMaterial() {
       _this.$http.get(interfaceStr+'cc/material/page?access_token='+token+'&page='+page+'&pageSize=6'+'&groupId='+_this.cateSele_val+'&materialType=0').then(function (res) {
       if(res.body.error_code===200){
       _this.pic_list_arr = res.body.resultObj.data;
       _this.total = res.body.resultObj.count;
       var str='';
       ...
       str+='<li class="pic_list">'
       +'<input type="checkbox" name="pic_list_radio" class="pic_list_radio" v-model="'+_this.img_checked+'" value="'+data.id+'">'
       +'<img src="'+data.materialUrl+'" alt="" id="img" class="pic_list_video">'/!*'+reader.result+'*!/
       +'<span class="img_name">'+data.fileName+'</span>'
       +'<div class="img_edit_box clear">'
       +'<i class="fa fa-edit" id="fa_edit"></i>'
       +'<i class="fa fa-trash-o" id="fa_trash_o"></i>'
       +'</div>'
       +'</li>';
       }
       $('.pic_tab').html(str);
